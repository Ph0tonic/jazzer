...
public class Builder {
  ...
  static Map<Class<?>, Object[]> prebuiltObjects = new HashMap<>();
  
  public static Object build(FuzzedDataProvider data, Class<?> type) {
    return consume(data, type, null, false);
  }

  // public static List<Object> build(FuzzedDataProvider data, List<Class<?>> types) {
  //   return Arrays.stream(types).map(type -> build(data, type)).collect(Collectors.toList());
  // }
  ...
  public static Object consume(FuzzedDataProvider data, Class<?> type, AutofuzzCodegenVisitor visitor) {
    return consume(data, type, null, true);
  }

  static Object consume(FuzzedDataProvider data, Class<?> type, AutofuzzCodegenVisitor visitor, Boolean nullable) {
    nullable = Optional.ofNullable(nullable).orElse(true);
    ...
    // We take the prebuilt object in 50% of the time
    if (prebuiltObjects.containsKey(type) && data.consumeInt(0, 1) == 0) {
      if (visitor != null)
        visitor.pushElement("prebuiltObjects.get("+type+")");
      Object[] objects = prebuiltObjects.get(type);
      return objects[data.consumeInt(0, objects.length - 1)];
    }
    ...
    if (nullable && !type.isPrimitive() && data.consumeInt(0, 19) == 0) {
      if (visitor != null)
